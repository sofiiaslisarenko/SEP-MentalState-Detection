...
########### für Modellvisualisierung
def plot_ridge_coefficients(model, features, n=25):
    # Koeffizienten-Datenrahmen erstellen
    coefs = pd.DataFrame({'feature': features, 'coef': model.coef_})
    # Sortieren nach absolutem Wert (Stärke der Gewichtung)
    coefs = coefs.reindex(coefs['coef'].abs().sort_values(ascending=False).index).head(n)

    plt.figure(figsize=(10, 6))
    sns.barplot(data=coefs, x='coef', y='feature', palette='coolwarm')
    plt.title(f"Top {n} Koeffizienten (Ridge) - Einfluss auf Sentiment", fontsize=14)
    plt.xlabel("Gewicht des Features")
    plt.tight_layout()
    plt.show()

def plot_tree_importance(model, features, name, n=25):
    # Wichtigkeit extrahieren
    importances = model.feature_importances_
    # In DataFrame sortieren
    feat_imp = pd.DataFrame({'feature': features, 'importance': importances})
    feat_imp = feat_imp.sort_values('importance', ascending=False).head(n)
...
# Visualisierung der Entscheidungsmechanismen
    print("\nGeneriere Feature-Importance-Plots...")

    # 1. Ridge: Zeigt Richtung und Stärke
    plot_ridge_coefficients(model_ridge, feature_cols)

    # 2. Random Forest: Zeigt Fokus
    plot_tree_importance(model_rf, feature_cols, "Random Forest")

    # 3. XGBoost: Zeigt Fokus
    plot_tree_importance(model_xgb, feature_cols, "XGBoost")


# SHAP

    # 1. Explainer initialisieren
    explainer_rf = shap.TreeExplainer(model_rf)
    explainer_xgb = shap.TreeExplainer(model_xgb)
    explainer_ridge = shap.LinearExplainer(model_ridge, X_train_scaled)
    ...
    shap_values_xgb = explainer_xgb.shap_values(X_test_scaled[:100], check_additivity=False)
    shap_values_ridge = explainer_ridge.shap_values(X_test_scaled[:100])

    # 3. Visualisierung
    print("Generiere Plots...")

    for values, name in [(shap_values_rf, "Random Forest"),
                         (shap_values_xgb, "XGBoost"),
                         (shap_values_ridge, "Ridge")]:
        plt.figure()
        shap.summary_plot(values, X_test_scaled[:100], feature_names=feature_cols, show=False)
